...
Publishing is a set rather than a choice, so a repository carrying both a Go binary and an iOS app gets one version and one tag for the pair (adr/0003).
`goreleaser` runs GoReleaser, `fledge` hands the archive to `service:stout/fledge`, and `none` still tags and writes a GitHub release.
